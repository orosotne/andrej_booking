...
import { CalendarDayPrint } from "./CalendarPrint";
...
export function CalendarScreen({
  ...
  // The day the print button exports — always a single day, tracking whichever
  // day the day/week view currently has in focus (defaults to today).
  const [printDay, setPrintDay] = useState<string>(() => todayIso());
    ...
    <div>
      <div className="no-print mb-4 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            title="Tlačiť / Export do PDF — zoznam pacientov na deň"
            aria-label="Tlačiť zoznam objednaných pacientov na deň"
            className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
          >
            <Printer className="h-4 w-4" />
            <span className="hidden sm:inline">Tlačiť deň</span>
          </button>
        </div>
      </div>
      ...
      {view === "month" ? (
        ...
        <CalendarView
          ...
          onActiveDayChange={setPrintDay}
        />
      )}

      <CalendarDayPrint iso={printDay} />
